...
const generateSchema = z.object({
    ...
    slideCount: z.preprocess((val) => (typeof val === 'string' ? parseInt(val, 10) : val), z.number().int().min(3).max(50).default(8)),
    theme: z.enum([
        'startup-pitch',
        'product-launch',
        'corporate-report',
    ...
    // Brand Kit & Templates
    brandColors: z.preprocess((val) => (typeof val === 'string' ? JSON.parse(val) : val), z.object({
        primary: z.string(),
        secondary: z.string(),
        accent: z.string(),
        background: z.string(),
    ...
    brandFonts: z.preprocess((val) => (typeof val === 'string' ? JSON.parse(val) : val), z.object({
        heading: z.string(),
        body: z.string(),
    }).optional()),
    brandLogoUrl: z.string().optional(),
    templateOverlay: z.preprocess((val) => (typeof val === 'string' ? JSON.parse(val) : val), z.object({
        logo: z.object({
...
let GenerateController = class GenerateController {
        ...
        if (data.parseToken) {
            ...
            if (parsedRaw) {
                ...
                if (data.sectionIds && data.sectionIds.length > 0) {
                    sectionsToUse = sectionsToUse.filter((s) => data.sectionIds.includes(s.id));
                }
                ...
                data.sectionMeta = sectionsToUse.map((s) => ({
                    id: s.id,
                    title: s.title,
                ...
                if (data.sectionVisuals) {
                    data.sectionVisuals = data.sectionVisuals;
                }
            }
        }
     ...
     */
    async storeParsedDocument(parsed, userId) {
        const token = ulid();
        await this.redis.set(`parsed:${token}`, JSON.stringify({ ...parsed, userId }), 'EX', 3600 // 1 hour expiry
        );
        return token;
    }
};
...
__decorate([
    ...
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", Promise)
], GenerateController.prototype, "parseDocument", null);
