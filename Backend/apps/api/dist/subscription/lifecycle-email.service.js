...
const INACTIVITY_REACTIVATION_SCHEDULE = [
...
const CANCEL_SEQUENCE_SCHEDULE = [
    { emailType: 'cancel_confirmation', offsetMs: 0 },
    { emailType: 'cancel_day3_winback', offsetMs: 3 * DAY_MS },
];
let LifecycleEmailService = LifecycleEmailService_1 = class LifecycleEmailService {
    prisma;
    queues;
    opsService;
    logger = new Logger(LifecycleEmailService_1.name);
    constructor(prisma, queues, opsService) {
        this.prisma = prisma;
        this.queues = queues;
    ...
    async schedulePackBalanceAlert(params) {
        ...
        const packScopeKey = latestPack?.payload?.scopeKey ||
    ...
    async scheduleSingleEmail(params) {
        ...
        const payload = {
            ...
            footerReason: templateDefinition?.kind === 'marketing'
    ...
    sanitizeScopeKey(value) {
        return value.replace(/[^a-zA-Z0-9_-]/g, '-');
    }
    applyFlowWindow(date, flowRuntime) {
        if (!flowRuntime) {
            return date;
        }
        const scheduled = new Date(date);
        const [startHour, startMinute] = flowRuntime.sendWindowStart.split(':').map((value) => Number(value));
        const [endHour, endMinute] = flowRuntime.sendWindowEnd.split(':').map((value) => Number(value));
...
LifecycleEmailService = LifecycleEmailService_1 = __decorate([
    Injectable(),
