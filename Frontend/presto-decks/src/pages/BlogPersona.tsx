...
import { BlogPost, getPersonaDescription, getPersonaLabel, getPostsByPersona } from "@/lib/blog";
...
export default function BlogPersona() {
  ...
  useEffect(() => {
    const loadPosts = async () => {
      ...
      try {
        setLoading(true);
        const data = await getPostsByPersona(persona, i18n.language);
        setPosts(data);
      } finally {
        setLoading(false);
      }
  ...
  return (
    <div className="min-h-screen pt-24 pb-16 px-4">
      <SEO title={`${personaLabel} PowerPoint IA`} description={description} url={`/blog/metier/${persona || ""}`} />
      <Helmet>
        <script type="application/ld+json">
          {JSON.stringify({
            ...
            itemListElement: [
              ...
              { "@type": "ListItem", position: 3, name: personaLabel, item: `https://www.slideai.fr/blog/metier/${persona}` },
            ],
          })}
        </script>
      </Helmet>
